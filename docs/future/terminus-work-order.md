...
├── internal/
│   ├── config/                     # Config struct + dd cascade (incl. ~/.config/terminus/config.yaml); canon path, reviewer, log_destination (~/terminus, user-level)
│   │   └── config.go
...
2. **terminus scaffold + config + cli.** cobra root/serve/version/monitor; `Config` (`dd` cascade incl. `~/.config/terminus/config.yaml`: canon path, default reviewer, `log_destination` default `~/terminus`, user-level outside the repo); `dl.Init` with trim prefix; empty MCP server that starts over stdio. *Verify:* `go build`; `terminus serve` starts and responds to MCP initialize; `terminus version`.
