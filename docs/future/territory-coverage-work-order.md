...
Grounded against commit `d2f4925`, amended during stage 1 review on 2026-09-08. Implements [territory coverage](territory-coverage.md). The spec owns behavior and rationale; this document owns code placement, data contracts, stages, and validation. Mercurius returned `ready_to_build` on the original pair. Michael subsequently required `dd` to own MCP payload binding as well as persistence, superseding the original JSON-tag and typed-handler mechanisms. Stage 1 is submitted but requires the binding correction below before acceptance; the earlier verdict does not review this amendment.
...
| `internal/mcpserver/mcpserver.go` | Replace typed SDK payload binding with one `dd` adapter; preserve tool names, discovery, and structured error mapping. |
...
No dependency upgrade is planned. Use the pinned harness v0.1.1, df v1.0.3, MCP SDK v1.5.0, and google/jsonschema-go v0.4.2. `dd` owns all project-struct binding and unbinding, including MCP tool inputs, results, and errors. The SDK owns the MCP/JSON-RPC envelope around already-unbound payloads. Project-owned structs carry no `json` tags, and tool schemas must not depend on JSON-tag inference. Schema descriptions live in explicit MCP schema definitions.
...
Every new review status, completed result, collect response, and audit result carries `coverage`. Historical status/result DTOs use a nullable Go pointer, which `dd` omits when nil, so absence survives reading and re-emission. Never fill missing historical coverage from the current canon. New ad-hoc reviews have an explicit object, not a nil pointer.
...
Use a pointer for `file_counts`, omitted for not-assessed output; initialize all array fields to empty slices during conversion. Use `dd:",+omitempty"` for optional scalar fields such as `reason`; keep `assessed` and required arrays free of that flag. df v1.0.3 omits nil pointers and emits slices as arrays during unbind. Use that same `dd` output for disk and MCP, including unmatched exclusions with `files: []` and coverage passed through cloning. Do not test a second project-struct encoding path through `json.Marshal`. The shared binding adapter below preserves raw reviewer JSON without turning it into a byte array. Test actual `dd` and MCP serialization, including nil historical coverage and explicit false assessment, rather than relying on Go struct equality alone.
...
Use a pointer to an initialized slice, or an equivalent explicit presence representation in the transport adapter, to preserve requested-empty versus omitted `coverage_map` through `dd` into both disk JSON and MCP payloads. Map detail is optional for payload size; it does not affect counts, uncovered files, or dead-pattern results. Do not truncate lists silently or require agents to parse human-readable text to recover paths.

## Shared `dd` binding and MCP adapter

This correction belongs to stage 1, before review-time coverage is connected. Migrate existing `start_review` and `collect_review` to the shared adapter now; stage 3 registers `audit_coverage` through the same path. Remove `json` tags from project-owned DTOs in report, broker, monitor, findings, errs, and mcpserver, updating any tests that depended on direct struct JSON encoding. Binding names use `dd`'s snake-case default or explicit `dd` names. Preserve existing tool field names, service defaults, error categories, and historical record readability. If a legacy MCP field and its disk counterpart have different presence requirements, express that at the transport boundary with a small `dd` DTO; do not retain parallel encoders on one struct.

Add a shared registration helper in `internal/mcpserver/binding.go` around `(*mcp.Server).AddTool`, whose raw handler receives `req.Params.Arguments`. This SDK entry point does not bind inputs, validate schemas, populate success content, or translate handler errors for us. The helper owns those responsibilities once for all tools:

1. Treat absent arguments as an empty object. Decode supplied arguments with `dd.DecodeStrictJSON`, validate the resulting map against the registered input schema, and bind the validated map into the project input struct with `dd.Bind` and `dd.Strict()`. Do not validate by unbinding a zero-filled struct: doing so loses the distinction between a missing property and an explicit false or empty value. Malformed types, duplicate keys, non-object arguments, and schema/binding failures become useful `user_error` tool results. Service code retains semantic defaults and validation; schema descriptions document those defaults without silently rewriting the request.
2. Call the service, then unbind the success DTO through `dd.Unbind`. Validate that map against the resolved success schema when the tool declares one. Set `StructuredContent` to the unbound map and emit the corresponding `dd` JSON in `TextContent` for clients that consume text. Only maps, lists, scalars, or opaque JSON already produced by `dd` may cross into SDK payload serialization. Never pass our DTO to the SDK for inferred binding or marshaling.
3. Convert service errors through the existing category mapping and unbind the project error DTO with `dd` before setting `StructuredContent`. Retain `IsError: true` and the exact `{error: {code, message, details}}` shape, including optional `at` where already used. Validate success data only on success; do not subject an error envelope to the success schema. Binding/output-validation failures caused by our implementation are `internal_error`, not empty success responses. Preserve the existing distinction between classified tool errors and protocol errors, and encode project data in `jsonrpc.Error.Data` through `dd` too.

Define schemas explicitly in `internal/mcpserver/schemas.go` with the pinned `jsonschema.Schema` types. Reuse schema constructors for quality refs, coverage, counts, exclusions, errors, and map entries; there is one coverage definition shared by audit and review responses. Property names, requiredness, descriptions, and optional fields follow the wire contract. In particular, audit input requires `repo_path`; `include_map` is an optional boolean with its explanatory description; `coverage_map` is optional and has array type when present. Preserve the existing start/collect discovery contract while replacing its inference mechanism. These finite schemas are transport declarations, not a second binder: do not build a generic reflection framework that reimplements `dd` naming/tag rules. Test agreement between the schemas and real `dd` output for the meaningful states and nested fields.

Resolve schemas once at registration with `Schema.Resolve`, validate maps with `Resolved.Validate`, and propagate schema-construction/resolution errors through server setup. Adjust `New`, registration, and their CLI/test callers to carry errors rather than panic or silently register without a schema. `(*mcp.Server).AddTool` only checks the schema envelope; merely assigning `OutputSchema` is not runtime validation. Keep external schema references local so registration requires no network access. The lower-level handler also removes the typed-output overwrite problem behind advisory A1; its exact-error-payload and discovery checks remain required.

Move the existing raw-reviewer-JSON conversion/options out of broker-private persistence into a shared report codec used by both disk and MCP unbinding. Preserve raw JSON as its original nested value, including the existing historical nil behavior, rather than exposing the underlying byte slice. `json.RawMessage` as an opaque third-party carrier is distinct from binding project structs; keep any byte/value bridge confined to that codec, with `dd` owning the surrounding document. Do not add a project `json.Marshaler` or duplicate DTO solely to restore SDK struct serialization. Use the same codec for structured and text success output so neither diverges on raw reviewer content.
...
Register `audit_coverage` on the existing broker-backed MCP server. Input struct: `repo_path` required, `rubric` optional, `include_map` optional boolean. Put field descriptions in the explicit input schema, particularly for `include_map`; describe defaults and map use in the tool description. Update `collect_review` and `start_review` descriptions to explain coverage reporting and direct agents to the audit. Set the audit tool annotations to `readOnlyHint: true` and `destructiveHint: false`; the pinned SDK supports both. Set `DestructiveHint` to a non-nil pointer to false so the explicit value is included in discovery. Exercise `tools/list`, not just a direct Go handler call, to verify discovery exposes the map option.

Assign the explicit audit success schema to `mcp.Tool.OutputSchema` and use the shared `dd` registration helper. Its success branch validates the unbound audit map, and its error branch preserves the separately unbound error envelope. Neither branch returns a project DTO through generic `mcp.AddTool` output serialization. Schema resolution is part of server setup, not a per-request operation.
...
Implement rubric exclusions and complete pattern validation, expose shared matcher/normalizer functions, add `internal/coverage` and `internal/report`, and pin domain-to-wire conversion. Complete the shared `dd` adapter correction above, including existing start/collect migration and removal of project JSON tags. Review execution does not begin calculating coverage until stage 2. Update current documentation and the changelog for any user-visible binding behavior delivered in this stage, such as strict malformed-request rejection.

Validate exact project-prefix membership and foreign/general exclusion, territory-free qualities, mixed covered/uncovered inputs, exclusions overlapping coverage and one another, empty and all-excluded scopes, no-local-tier scopes, malformed late pattern segments, and dead patterns across all rubric tiers including excluded-tree matches. Verify map false/true yields identical assessment and dead patterns, and map grouping preserves different matches within a directory. Test `dd` output and actual MCP payloads for empty arrays, optional counts/map, and historical nil coverage. Include unmatched exclusion file lists and cloned empty/ad-hoc assessments. Exercise start/collect, structured and protocol errors, raw reviewer JSON, input-schema validation before defaulting/binding, and schema discovery through a real MCP connection. Before the audit service exists, a test-only tool using the shared adapter and audit DTO/schema can verify unrequested-map omission and requested-empty-map `[]`. Assert required fields, exact snake-case keys, and preserved empty/false values against the declared schemas. Check project-owned Go structs have no `json` tags and no SDK typed binding path remains; an external protocol type is not a project DTO. Run targeted package tests followed by `make test`, then Terminus review to `clean` before the operator's stage acceptance.
