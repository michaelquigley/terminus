...
Terminus is a local MCP code-review broker. It reviews a finite changeset against a central canon of qualities and returns a `clean` verdict when there are no blocking findings.

## Orientation

The working conventions live in the grimoire. Start there at `AGENTS.md`, then read the implementation-agent guidance in `agents/agent-roles` and the design-build pipeline in `agents/design-build-pipeline`.

Current behavior lives in `docs/current/`. Forward-looking work lives in `docs/future/`.
...
Use `github.com/michaelquigley/df/dl` for logging and `github.com/michaelquigley/df/dd` for YAML/JSON marshaling and unmarshaling.
